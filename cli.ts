...
async function get() {
  ...
  try {
    ...
    if (err === null) {
      ...
      const lockfile = await fs.readFile('./flight.lock', 'utf8')
      ...
      for (let x in json) {
        ...
        const start = async () => {
          const download = (await stream(urlformat)).pipe(createWriteStream(`./.flight/${name}-${version}.tgz`));
          ...
          if (find == true) {
            await execAsync(`cd .flight ; tar zxvf ${name}-${version}.tgz -C ../node_modules`);
          }

          if (find == false) {
            execAsync(`mkdir node_modules ; cd .flight ; tar zxvf ${name}-${version}.tgz -C ../node_modules`);
          }
      ...
      for (let x in json2) {
        ...
        if (raw.startsWith("@") == false) {
          ...
          const start = async () => {
            ...
            if (find == false) {
              await execAsync(`mkdir node_modules ; cd .flight ; tar zxvf ${name}-${version}.tgz -C ../node_modules`);
            }
